...
import { push, type Role } from "@/api/system/role";
  ...
  <ElDialog v-model="dailogVisible" draggable width="40%" :title="formData.id ? '修改' : '添加'">
    <ElForm :model="formData" ref="formRef" label-width="80px">
      ...
      <ElFormItem label="描述" prop="desc" :rules="[]">
        <ElInput type="textarea" v-model="formData.desc" show-word-limit resize="none" placeholder="请输入角色描述" :maxlength="600" :rows="10"></ElInput>
      </ElFormItem>
    </ElForm>
    <template #footer>
